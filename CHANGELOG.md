...
- Fixed response-recovery countdowns during long ChatGPT tool/reasoning turns: YOLO now tracks the latest conversation-turn activity separately from the final assistant message and restarts the 3-minute recovery deadline whenever that response turn visibly advances, so active page progress cannot be mistaken for a dead response.
