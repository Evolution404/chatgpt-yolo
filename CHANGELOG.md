...
- Added an in-page live YOLO status window for active Goal/Loop workflows. It shows the current execution phase plus live countdowns for response recovery, response stabilization, 5-minute/10-minute/30-minute generation watchdog thresholds, Stop grace refresh, queue scheduling, periodic refresh, and runner lease renewal; the workflow bar also shows the next timed action at a glance.
