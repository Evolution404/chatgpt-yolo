...
Response-start recovery is based on **lack of observable response progress**, not only on the final assistant message. For ChatGPT tool/reasoning turns, YOLO snapshots the latest conversation turn when it queues the workflow prompt and persists subsequent turn activity. Each visible advance moves the recovery deadline forward; a page that is still adding reasoning/tool progress therefore cannot hit the 3-minute refresh deadline merely because the final assistant message has not settled yet.
